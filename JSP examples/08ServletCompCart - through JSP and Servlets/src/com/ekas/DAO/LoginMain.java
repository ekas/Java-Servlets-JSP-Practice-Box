...
public class LoginMain implements interDao{
	...
	public List<product> searchAllProd() {
		try {
			...
			String query = "SELECT * from prodInventoryEk";
			try {
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery(query);
				product prod = new product();				 				
				while (rs.next()) {
					prod.setItemName(rs.getString("ProdName"));
					prod.setItemQty(rs.getFloat("ProdQty"));
					prodList.add(new product(rs.getString("ProdName"), rs.getFloat("ProdQty")));
				}
				return prodList;
			} catch (SQLException e) {
				e.printStackTrace();
